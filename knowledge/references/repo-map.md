...
| `web/src/` | фронт: `api/` (клиент), `auth/` (вход и права), `app/` (оболочка, маршруты, разделы), `pages/` | меню строится по правам из `/api/auth/me`, а не по названию группы |
| `deploy/` | `nginx.conf` — nginx контейнера фронта; `proxy/ahrefs-cases.conf` — сайт nginx хоста: TLS, пароль прокси, потолок частоты входа | на сервере лежит копия прокси со строками certbot — правку шаблона переносят руками. Имя сайта в шаблоне — заглушка: репозиторий публичный |
| `docs/` | часть в git (находки, экономия, чеклисты), часть вне (ТЗ, планы фаз) | клиентское — вне git по варианту D |

**Настроек проекта два файла, и это не дубль.** Корневой `pyproject.toml` —
сборка, зависимости и все инструменты. `src/pyproject.toml` — **только** область
мутационного гейта: mutmut 3.x запускается из корня импортов (`src/`, так его
зовёт `scripts/lint/mutation_py.sh`) и читает конфиг из своего каталога, поэтому
в корне эта секция не читается никем. Ключ там `source_paths`; прежнее имя
`paths_to_mutate` из mutmut 2.x для 3.x означает отсутствие настройки, и гейт
из-за него четыре дня отказывался судить (Z18–Z20, 15.09.2026). Ничего, кроме
`[tool.mutmut]`, в `src/pyproject.toml` класть нельзя: pytest и coverage читают
корневой, и второй набор тех же ключей разошёлся бы с ним молча.
...
- **Зависимость без верхней границы приезжает в CI свежей в любой день.**
  24.09.2026 между двумя прогонами в четыре минуты вышел SQLAlchemy 2.1.0: он
  объявил устаревшим `select().distinct(выражение)` (DISTINCT ON в
  `cache.share_twin_points`), `filterwarnings = ["error"]` дал 47 падений на
  ветке, чей дифф этого не трогал, а локальный venv на 2.0.52 был зелёным. В
  `pyproject.toml` стоит `<2.1` до перехода на `postgresql.distinct_on`, которого
  в 2.0 нет. Различитель — строка `Downloading sqlalchemy-…` в логе установки CI.
...
- **WeasyPrint требует системных библиотек, и не только очевидных.** cairo и
  ...
  пакетным менеджером.
- **Скрытый пункт меню — не защита.** Разделы во фронте фильтруются правами
  (`web/src/app/nav.ts`), но серверные проверки стоят и остаются: кнопка,
  которой не видно, экономит человеку клик, а не закрывает доступ.
...
- **Устаревшая константа Starlette роняет обработчик ошибок.**
  `HTTP_422_UNPROCESSABLE_ENTITY` и `HTTP_413_REQUEST_ENTITY_TOO_LARGE`
  объявлены устаревшими, а `filterwarnings = ["error"]` превращает
  предупреждение в исключение — то есть ответ приходит пятисоткой, и только на
  пути обработки ошибки. Брать нынешние имена (`HTTP_413_CONTENT_TOO_LARGE`).
...
- **Окна точек передаёт вызывающий, и вызывающих трое.** `collect` не знает про
  пороги по контракту слоёв, поэтому окна читает `classify.windows.point_windows`
  — один источник для CLI, фоновой задачи и сметы. Пока источников знания было
  два, очередь звала сбор без окон: прогон, запущенный кнопкой, покупал
  бесплатный максимум, а прогон из консоли — окна порогов.
...
- **`docker compose config` печатает значения.** Доехала ли переменная до
  контейнера — `grep` по имени; целиком вывод смотрят только в дереве с
  фальшивым `.env`, иначе в нём ключ Ahrefs.
- **Порт на `0.0.0.0` докер открывает мимо ufw.** Фронт публикуется только на
  петлю (`WEB_BIND`), наружу сервис отдаёт nginx хоста.

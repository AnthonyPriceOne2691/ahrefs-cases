...
**Версии зависимостей заданы только нижней границей, lock-файла нет** — CI и сборка
образа ставят то, что вышло в момент сборки. Исключение одно: `sqlalchemy[asyncio]<2.1`
(24.09.2026 вышел 2.1.0, `.distinct(expr)` для DISTINCT ON в `collect/cache.py` стал
устаревшим, и `filterwarnings = error` уронил 47 тестов CI). Снимать границу — только вместе
с переходом на `postgresql.distinct_on` везде; по существу нужны зафиксированные версии (Z48).

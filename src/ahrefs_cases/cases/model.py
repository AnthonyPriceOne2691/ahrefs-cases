...
class CaseOutcome(StrEnum):
    """Чем закончилась попытка собрать кейс. Исходов пять, а не два.

    «Не положен по группе», «данных не хватило», «вердикта нет» и «вердикт не
    про эти данные» ведут к разным действиям — ничего не делать, докупить
    историю, классифицировать, переклассифицировать по рядам вердикта, — и
    слияние их в «кейса нет» оставляет человека без следующего шага (L32, L34).
    Каждый исход ступень кейсов пишет в журнал прогона своим значением
    (`RunItemOutcome.CASE_…`, собранный — `ok`).
    """
    ...
    """Вердикт посчитан не по тем рядам, которые к нему показывают.

    Три случая, одно действие: источник вердикта другой, источник не записан
    вовсе (вердикт старше поставки) или числа точек не воспроизводятся по
    серии — ряды докупили после решения. Кейс в таком состоянии собрался бы из
    двух разных миров: таблица из одного, кривая из другого (Z10). Лечится
    бесплатным `classify` по рядам, по которым вынесен вердикт, — не по
    текущему режиму; чем именно не сошлось, говорит `CaseAttempt.detail`."""
...
class CaseData:
    ...
    highlights: tuple[Change, ...] = ()
    """Что подсвечено: правило выбора — `cases.highlights`. Подсветка попадает и
    в плитки листа, и в текст, поэтому решается один раз здесь."""

    narrative: str = ""
    """Текст кейса по числам. Пусто — законное состояние: кейс собран, но текст
    не просили (например, при предпросмотре списка командой `cases`)."""
    ...
    domain: str = ""
    """Домен проекта **всегда**, даже когда он скрыт в заголовке.

    Нужен одному — имени файла. Решение владельца 15.09.2026: файл называется
    доменом и у скрытых проектов тоже, потому что найти нужный PDF среди
    десятка «сайт в нише travel — Кейс (2).pdf» невозможно. Анонимность при
    этом **остаётся внутри листа**: заголовок, текст и графики скрытого проекта
    его по-прежнему не называют. Это отступление от ТЗ, названное вслух.

    Пусто — законное состояние: кейсы, собранные до этой правки, и предпросмотр;
    имя тогда падает обратно на заголовок, как было.
    """

    version: int = 0
    """Номер сборки. В имени файла он разводит пересборки одного проекта; до
...
class CaseAttempt:
    ...
    """Чья попытка — у каждого исхода: журнал прогона пишет судьбу по номеру
    проекта, домен у двух кампаний одного сайта один (Z39). `None` — только у
    попыток, собранных руками в тестах."""

    verdict_id: int | None = None
    """Откуда кейс: нужно записи. `None` у исходов, где кейса нет."""
    ...
    detail: str = ""
    """Почему исход такой — словами, для человека. Пусто у исходов, которые
    объясняются своим именем. У `verdict_mismatch` заполнено всегда: «кейса
    нет» без причины отправляет человека искать поломку там, где сработало
    правило (класс уроков L32, L34)."""
...
class CaseReport:
    ...
    def as_lines(self) -> list[str]:
        counts = {
            ...
            "вердикт не про эти данные": CaseOutcome.VERDICT_MISMATCH,
        }
        lines = [

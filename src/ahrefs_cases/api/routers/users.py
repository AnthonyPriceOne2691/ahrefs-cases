...
Четвёртая добавлена после прода: нельзя лишить себя права управлять людьми —
ни личным «Нет», ни сменой своей группы. Та же дверь, запертая изнутри, только
другим ключом: защита «последнего администратора» смотрела на группу, а доступ
теряется и личным решением.
"""
...
async def patch_user(
    ...
    if user.id == actor.id:
        _refuse_losing_own_management(user)
...
def _refuse_losing_own_management(user: User) -> None:
    """Себя не лишить права управлять людьми: это делает только другой управляющий.

    Прод, 24.09.2026: единственный инженер поставил себе «Нет» у «добавлять
    пользователей» — и вернуть право стало некому, экран людей отвечал 403.
    Защита «последнего администратора» этого не видела: она считает группу
    `admin`, а доступ теряется и личным решением, и сменой своей группы.

    Правило строгое по решению владельца — действует и при живом втором
    управляющем, как запрет выключить себя. Отсюда и гарантия: API отвечает
    только управляющему, а он себя не лишит права, не выключит и не удалит,
    значит, управляющий в сервисе остаётся всегда — считать «последнего» не
    нужно.

    Смотрит на итог после правки — `rights_of_user`, та же функция, что решает
    доступ; правило перекрытия группы личным решением второй копии не заводит.
    Правка к этому моменту уже лежит в объекте, но отказ откатывает сессию.
    """
    if "manage_users" not in rights_of_user(user):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                "нельзя лишить себя права «добавлять пользователей»: "
                "это может сделать только другой управляющий"
            ),
        )

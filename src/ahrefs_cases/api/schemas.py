...
class RunRow(BaseModel):
    ...
    Есть у сборок кейсов, собравших архив, начиная с 25.09.2026. У прежних
    копии нет: пачка дня одна, и каждая сборка её переписывала."""

    pack_cases: int = 0
    """Сколько кейсов в копии пачки прогона; 0 — копии нет."""

    current_stage: str = ""
    """У строки цикла по файлу — ступень, которая идёт или шла последней
    (`stage1`, `stage2`, `case_data`, `cases`); у прочих строк пусто."""

    build_cases: bool = False
    """Прогон предлагает «Собрать кейсы»: это последний «данные под кейс», он
    закончился, и сборки после него не было. Решает сервер: экран видит одну
    страницу журнала, а «последний» считается по всем прогонам."""
...
class RunItemView(BaseModel):
    ...
    stage: str = ""
    """Ступень цикла, к которой относится судьба: у строки цикла в раскрытии
    судьбы всех его ступеней, и без ступени «пропущен» шага 1 и сборки
...
class IntakeReportView(BaseModel):
    ...
    """Проекты этого источника: созданные и обновлённые. По ним экран считает
    смету цикла и запускает его — прогон по файлу, а не по всей базе."""
...
class CycleStart(BaseModel):
    """Проекты цикла по файлу — те, что вернул приём списка (`project_ids`)."""

    project_ids: list[int] = Field(min_length=1, max_length=1000)

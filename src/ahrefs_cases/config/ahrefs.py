...
class AhrefsSettings(Settings):
    ...
    цена запроса."""
    units_counter_lag_hours: int = Field(
        24, ge=0, le=168, validation_alias="AHREFS_UNITS_COUNTER_LAG_HOURS"
    )
    """За сколько часов вычитать из ответа Ahrefs собственный журнал расхода.

    Замер 13.09.2026: счётчик `units_usage_api_key` реален (за сутки 530 812 →
    532 476), но **отстаёт** — 50 units, потраченные минутами раньше, в нём не
    видны. Резерв прогона эту дыру не закрывает: он снимается по статусу
    прогона, то есть раньше, чем обновляется счётчик.

    Двадцать четыре часа — **верхняя граница замера**, а не догадка: сдвиг за
    сутки виден, сдвиг за минуты — нет. Ошибка в эту сторону стоит лишнего
    отказа, ошибка в обратную — прогона, оборванного на середине.

    Ноль выключает вычет, и это осознанный режим для тарифа, где счётчик
    обновляется сразу."""
    ...
    **вперёд** от `period_start` (`classify/points.py`), а расчёт baseline'а
    ...
    """Два endpoint'а шага 2, выключенные по умолчанию, — по 50 units за домен каждый.

    DR отсюда ушёл 11.09.2026: он must-have по ТЗ, и выключать требование ТЗ
    флагом — способ забыть о нём молча. Теперь покупается ступенью кейса двумя
    точками, только тем, у кого кейс будет (1000 units против 3000).

    Ни `pages`, ни `search_volume` не участвуют в правилах классификации и не
    входят ни в блоки кейса по ТЗ (гео, период, задача, что сделали, А → Б по
    трафику, позициям и ссылкам), ни в must-have список метрик. На тридцати
    кандидатах это 2000 units за данные, которые никто не читает.
    ...
    checkpoint_every: int = Field(1, ge=1, le=1000, validation_alias="COLLECT_CHECKPOINT_EVERY")
    ...
    По умолчанию каждый домен (24.09.2026): в той же транзакции лежит журнал
    оплаченных запросов, и при 10 убитый воркер (выкатка, OOM) терял до девяти
    оплаченных записей — экран расхода показывал меньше потраченного, а остаток,
    от которого preflight разрешает прогон, выходил оптимистичнее правды.
    Коммит на домен стоит миллисекунды; сотня доменов — сотня коммитов."""

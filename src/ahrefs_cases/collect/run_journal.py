...
CHAIN = "chain"
"""Метка прогона, открытого кнопкой «Довести до кейсов» (решение владельца
25.09.2026), — в снимке параметров. Ступени цепочки, открытые воркером, несут
её сами; «данные под кейс» открывает `_stage2` без неё, и принадлежность к
цепочке узнаётся по общему ключу задачи (`params_snapshot["job"]`)."""

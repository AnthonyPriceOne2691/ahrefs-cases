...
class HistoryRequest:
    ...
    """Конец периода работ проекта. Живому Ahrefs не нужен — у него значение
    месяца не зависит от того, каким окном его спросили.

    Нужен фикстуре: она рисует форму кривой **по периоду работ**, и без якоря
    считала бы её от конца запрошенного окна. Тогда один и тот же месяц
    приходил бы разным числом в зависимости от схемы сбора — а схема выбирается
    по цене. Найдено 12.09.2026: замер цен переключил ссылающиеся домены с
    истории на две точки, и вердикт проекта поехал следом (good → medium),
    хотя данные о домене не менялись."""
...
class AhrefsProvider(Protocol):
    ...
    def source(self) -> MetricSource:
        """Чем помечать точки в базе.

        `fixture` и `live` не смешиваются в одной серии: иначе непонятно, за
        какие данные заплачено. Объявлено свойством, а не атрибутом: атрибут в
        протоколе требует, чтобы его можно было **присвоить**, и обёртка
        (`SingleFlightProvider`), которая лишь пробрасывает чужой источник,
        протоколу не удовлетворяла бы. Менять источник провайдера извне никто
        не должен, так что чтения достаточно.
        """
        ...

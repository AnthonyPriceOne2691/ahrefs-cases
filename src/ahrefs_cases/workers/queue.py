...
class InlineQueue:
    ...
    **Граница применимости, оплаченная прогоном в контейнере: только там, где
    движок базы не разделяется с другим циклом событий.** Задача поднимает свой
    цикл в отдельном потоке, а движок кэширован на процесс и его соединения
    привязаны к циклу, в котором созданы. В сервере (uvicorn) это даёт
    `asyncpg InternalClientError: got result for unknown protocol state` —
    соединение из чужого цикла. В тестах не проявляется: там движок собран без
    пула, и каждая сессия открывает новое соединение в своём цикле; в CLI
    процесс один и цикл один.

    Значит для сервера бэкенд обязан быть `redis`: задача уходит в отдельный

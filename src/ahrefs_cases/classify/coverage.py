...
def gap(
    ...
    **Различаются два вида отсутствия, и это главное в модуле.** Месяц внутри
    ...
    Отсюда достижимый случай ровно один — месяцы **вне периода работ**, которые
    просит `pre_start_baseline_months`: шаг 1 собирается историей, то есть весь
    период покупается целиком. Месяц внутри периода, которого нет в серии, —
    это не пробел в покупке, а отсутствие данных у Ahrefs; о нём говорит
    `rules._truncated_history` справочной записью, и группу она не меняет.

    **Граница проведена по периоду, а не по краям собранной серии.** Прежняя
    редакция считала непокупленным любой месяц вне серии, и живая проверка
    13.09.2026 показала цену: у девяти проектов из девятнадцати не было первого
    месяца периода (Ahrefs его не знает), и после активации версии порогов они
    остались **без вердикта вовсе** — карточка пустая, причина нигде. При этом
    `classify` тем же порогам давал им группы: правило стояло в обёртке
    `evaluate`, и короткий путь его не видел.

    Проект, собранный двумя точками, в эту проверку не попадает: у него не
    хватает середины периода, а это Z6, и судит о нём правило достоверности
    серии.
    """
    ...
    def unbought(months: list[date]) -> tuple[date, ...]:
        return tuple(m for m in months if m not in known and not (period_start <= m <= period_end))

...
DEFAULT_BREAKERS = {"max_files_touched": 25, "max_loc_diff": 800,
                    ...
                    "max_runtime_paths": 1,
                    # §3.4a: единственный breaker, который считает не объём и не
                    # количество, а ПЕРЕХОД состояния. Ноль значит «ни одной
                    # необратимой поверхности, до которой агент дотягивается без
                    # человека». Пока такой нет — он молчит и его не видно;
                    # появилась — остановка. Его waiver текстом НЕ принимается:
                    # он охраняет ровно класс «текстовый waiver подделывается».
                    "max_unsigned_irreversible": 0}
...
# ВТОРОЙ принцип исключения — «сгенерированное, а не написанное». Пути выше
# собраны по первому («механика контура»), и lock-файлы под него не подходят:
# они ПРОДУКТОВЫЕ. Их объём — функция числа пакетов, а не риска правки, и
# построчно их не читает никто. Риск новых зависимостей уже судит другой гейт,
# точнее нацеленный: `check_new_dependency.py` требует строку `new_dependency:`
# в STATUS на каждую ПРЯМУЮ зависимость, с причиной и автором.
#
# Замер, на котором это найдено (portfolio-site, поставка «механика основания»;
# обратный прогон канона 2026-08-16): дифф +859/-12 при лимите 800 — ПРОБИТ, и
# 564 строки из них дал один `package-lock.json` от четырёх devDependency. Лимит
# выбирался до написания первой строки вёрстки. С исключением: net 283.
#
# Прежний счёт создавал ещё и вредный стимул: дешевле НЕ объявлять зависимость,
# чем объявлять, — объявленная прямая тянет lock в blast radius, транзитивная
# приезжает молча.
#
# ⚠ Сверяется ИМЯ ФАЙЛА, а не префикс пути, и это НЕ то, что написал проект.
# Он починил у себя строкой `"package-lock.json"` в кортеже выше: на его
# раскладке lock лежит в корне, и `startswith` совпадает. На флоте так лежит
# ОДИН проект из трёх — у двух других `frontend/package-lock.json` и
...
def out_of_blast_radius(path: str) -> bool:
    """Путь не считается в breaker'ах §3.4: механика контура ИЛИ сгенерированное.

    Один вход на оба принципа, потому что потребителей у списка двое — фильтр в
    `diff_stats` и печать состава в `delivery_check`. Разъехаться им нельзя:
...
# контура и совпадает с `NOT_OUR_CODE` в `check_ast_gate.py`; согласие трёх

...
case "$RULE" in
  ...
  di-indirection)
    ...
    ;;
  service-no-web)
    PATTERN='^[[:space:]]*(from fastapi|import fastapi|from starlette|import starlette|import rq|from rq)'
    # Адаптация под раскладку проекта (объявлена в adapted.json): сервис-слоя с
    # именем `services/` здесь нет — ядро разложено по темам
    # (intake/collect/classify/cases/export/storage/config), и инвариант
    # «ядро не знает про обвязку» относится к ним. Канонный шаблон оставлен
    # рядом: если появится `services/`, он тоже будет судиться.
    FILTER='/services/|service\.py$|/ahrefs_cases/(intake|collect|classify|cases|export|storage|config)/'

...
- **reaper** — последний проход по прогонам был не дольше трёх минут назад.

**Реапер** (`workers/reaper.py`, отдельный контейнер, тик раз в минуту)
закрывает прогоны, чью задачу убили, и снимает замок «один активный прогон».
Без него прогон, убитый выкаткой посреди работы, оставался `running`
навсегда, а кнопка отвечала 409. Жива ли задача, он спрашивает у очереди: ждёт
она или её держит живой воркер. «Redis не ответил» — не «мертва». Поэтому
**выкатывать можно и посреди прогона**: прогон закроется с причиной «воркер
исчез», собранное до последнего домена сохранится (чекпойнт на каждом домене),
и повторный запуск купит только недостающее. Прогоны из консоли идут без
очереди, реапер судит их только по возрасту.

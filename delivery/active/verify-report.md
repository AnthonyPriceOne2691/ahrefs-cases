...
| После правки | `vitest run` | 192 passed (17 файлов); прежние 34 теста карточки прошли под `AuthProvider` без правок заглушек |
| Типы и стиль фронта | `tsc --noEmit`, `eslint src`, `prettier --check` | чисто; предупреждений eslint не прибавилось |
| Бэкенд целиком | `pytest -q` на своей копии базы `cases_delui_tests` | 682 passed, 3 skipped (бэкенд — `main` после #23, поставка его не меняет) |

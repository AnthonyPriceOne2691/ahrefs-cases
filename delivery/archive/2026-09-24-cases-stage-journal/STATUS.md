...
- **waivers:** none
- **observability:** 1
- **observe_signal:** на проде после ближайшей сборки кейсов кнопкой: журнал — «N из M», где N — число PDF в пачке и равно «кейс собран» минус «не попал» из строки `cases_packed` в логе воркера (Y1); в раскрытии строками только «данных не хватило», «вердикт не про эти данные», «вердикта этой версии нет», «не отдан: контент-запрет», а «не положен по группе» — числом (Y6); оповещение — «готова пачка кейсов «…»: кейсов внутри — N» (Y7); следующий сбор не тратит units на домены, пустоту которых помнит память (Y5)

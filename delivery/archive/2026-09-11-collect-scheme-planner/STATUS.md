...
- **runtime_paths:** src/ahrefs_cases/collect/scheme.py, src/ahrefs_cases/collect/plan.py reason=цена схемы проверяется только исполнением; доказательство — сквозной прогон на фикстурах со сверкой сметы и числа строк, живая цена — Ф7 п.6
- **observe_signal:** проект с периодом до девяти строк собирается историей, с десяти — двумя точками; смета сходится с числом строк, которое вернул провайдер; купленное окно не покупается второй раз

...
export interface IntakeReport {
  ...
  /** Проекты этого списка: по ним считается смета цикла и идёт цикл по файлу. */
  project_ids: number[];
}
...
export interface RunRow {
  ...
  mode: string;
  /** У прогона своя копия пачки — её отдаёт `GET /api/runs/{id}/pack`. */
  pack: boolean;
  /** Сколько кейсов в этой копии; 0 — копии нет. */
  pack_cases: number;
  /** Прогон предлагает «Собрать кейсы»: последний «данные под кейс», после
   *  которого сборки не было. Решает сервер — экран видит одну страницу. */
  build_cases: boolean;
  /** У строки цикла по файлу — ступень, которая идёт или шла последней. */
...
export interface RunItemView {
  ...
  /** Ступень цикла по файлу, к которой относится судьба; у прочих прогонов пусто. */
  stage?: string;
}

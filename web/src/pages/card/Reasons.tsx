 ...
 *
 * Прочерк означал два разных случая (Z25): историю метрики **не собирали** —
 * шаг 2 платится только кандидатам в кейсы — или собрали, а Ahrefs ничего не
 * отдал. Заказчик спрашивает «вы не купили или не смогли?», и разница в
 * деньгах: добрать можно только первое. Причину знает сервер (журнал расхода
 * units), экран её называет словом; молчит сервер — остаётся прочерк.
 *
 * Слово «не покупали» заменено на «не собирали» 16.09.2026: покупаются данные
 * Ahrefs, а читалось это как покупка самих ссылок и ключей — «ключи же
 * невозможно купить» (владелец). На уровне API признак остался `not_bought`:
 * там он про факт оплаты запроса, и журнал расхода отвечает именно за это.
 */
function factOf(row: ReasonRow): string {
  if (row.fact !== null) return num(row.fact);
  if (row.fact_missing === 'not_bought') return 'не собирали';
